...
describe('Login Router', () => {
  ...
  test('Should return 500 if no httpRequest had no body', async () => {
    const { sut } = makeSut();
    const httpRequest = {};
    const httpReponse = await sut.route(httpRequest);
    expect(httpReponse.statusCode).toBe(500);
  ...
  test('Should return 500 if no AuthUseCase if provided', async () => {
  ...
  test('Should return 500 if EmailValidator is provided', async () => {
    const authUseCaseSpy = makeAuthUseCase();
    const sut = new LoginRouter(authUseCaseSpy);
    const httpRequest = {
      body: {
